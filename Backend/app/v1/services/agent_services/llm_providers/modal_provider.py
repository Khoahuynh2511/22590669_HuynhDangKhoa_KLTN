    ...
    def get_llm(self, **kwargs: Any):
        ...
        if "model" not in model_kwargs:
            ...
            if agent_config.model and "/" in agent_config.model:
                ...
                logger.warning(
                    f"⚠️ Modal provider: Ignoring OpenAI model '{
                        agent_config.model}', using Modal model 'Qwen/Qwen3-8B-FP8'")
        ...
        logger.info(
            f"🤖 Creating Modal LLM with model: {
                model_kwargs.get('model')}, base_url: {
