...
"""
                ...
                logger.info(
                    f"🎯 RECOMMENDATION NODE: Formatting {num_shown} tours for Chat Agent (total found: {
                        len(packages)})")
                recommendation_message = f"""Tôi đã tìm thấy {
                    len(packages)} tour phù hợp, đây là top {num_shown} tour tốt nhất:
...
KHÔNG hỏi về ngày khởi hành - ngày đã được quy định trong package."""
            ...
            error_msg = "Xin lỗi, tôi không thể tìm tour recommendations lúc này. Vui lòng thử lại sau."
